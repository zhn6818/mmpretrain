...
这个模块提供了一个ClassInfer类，用于进行图像分类推理。
支持批量推理、单张图片推理等功能。
"""
...
class ClassInfer:
    ...
    用于加载训练好的分类模型并进行图像分类推理。
    支持单张图片推理和批量推理。
    
    Args:
        model_path (str): 模型文件路径
        device (str): 推理设备，默认为'cpu'
        class_names (Optional[List[str]]): 类别名称列表，如果为None则使用数字索引
    
    Example:
        >>> # 创建推理器实例
        >>> infer = ClassInfer('charCls/works/fullmodel_best.pth', device='cuda')
        >>> 
        >>> # 单张图片推理
    ...
    def infer_single(self, image_path: str, return_prob: bool = True) -> Dict[str, Any]:
        """
        对单张图片进行推理
        
        Args:
            image_path (str): 图片路径
            return_prob (bool): 是否返回概率分布，默认为True
            
        Returns:
            Dict[str, Any]: 推理结果，包含：
                - 'class_id': 预测的类别ID
                - 'class_name': 预测的类别名称
                - 'confidence': 预测置信度
                - 'probabilities': 所有类别的概率分布（如果return_prob=True）
        """
        try:
            # 预处理图片
    ...
    def infer_batch(self, image_dir: str, return_prob: bool = False) -> Dict[str, Dict[str, Any]]:
        ...
        Args:
            ...
            return_prob (bool): 是否返回概率分布，默认为False
